...
function install_and_activate_required_plugin() {
    ...
    $plugins = [
        ...
        [
            ...
            'file'     => 'stackable-ultimate-gutenberg-blocks/plugin.php',
            'url'      => 'https://downloads.wordpress.org/plugin/stackable-ultimate-gutenberg-blocks.latest-stable.zip',
        ]
    ];
    ...
    foreach ($plugins as $plugin) {
        ...
        if (!isset($electronics_retail_shop_installed_plugins[$plugin['file']])) {
            $electronics_retail_shop_install_result = $electronics_retail_shop_upgrader->install($plugin['url']);
            if (is_wp_error($electronics_retail_shop_install_result)) {
        ...
        if (!is_plugin_active($plugin['file'])) {
            $electronics_retail_shop_activate_result = activate_plugin($plugin['file']);
            if (is_wp_error($electronics_retail_shop_activate_result)) {
                wp_send_json_error([
                    'message' => "Failed to activate {$plugin['slug']}",
                    'error'   => $$electronics_retail_shop_activate_result->get_error_message(),
                ]);
            }
        }
    }
